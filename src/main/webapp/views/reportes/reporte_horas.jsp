...
<head>
    <meta charset="UTF-8">
    <title>Reporte de Horas (Resumen)</title>
    ...
    <div class="main-content">
        ...
        <div class="container">
            ...
            <div class="page-header" style="flex-wrap: wrap; gap: 16px;">
                <div style="flex: 1;">
                    <h1 style="font-family: 'Inter', sans-serif; font-weight: 600; font-size: 2rem;">Resumen de Horas</h1>
                    <p style="color: var(--md-sys-color-secondary); margin-top: 4px;">Totales de horas trabajadas acumuladas por empleado.</p>
                </div>
            </div>
                        ...
                        <table class="compact-table" style="width: 100%; min-width: 800px;">
                            <thead>
                                <tr>
                                    <th>Empleado</th>
                                    <th>Sucursal</th>
                                    <th style="text-align: right;">Hrs. Prog.</th>
                                    <th style="text-align: right;">Hrs. Trab.</th>
                                    <th style="text-align: right;">Hrs. Extra</th>
                                    <th style="text-align: right; color: #c62828;">Hrs. Deuda</th>
                            ...
                            <tbody>
                                <c:forEach var="r" items="${reporte}">
                                    <tr>
                                        ...
                                        <td style="text-align: right;"><fmt:formatNumber value="${r.totalHorasProgramadas}" type="number" maxFractionDigits="2"/></td>
                                        <td style="text-align: right; font-weight: 500;"><fmt:formatNumber value="${r.totalHorasTrabajadas}" type="number" maxFractionDigits="2"/></td>
                                        
                                        <td style="text-align: right; color: #2e7d32;">
                                            <c:if test="${r.totalHorasExtras > 0}">+</c:if><fmt:formatNumber value="${r.totalHorasExtras}" type="number" maxFractionDigits="2"/>
                                        </td>
                                        
                                        <td style="text-align: right; color: #c62828; font-weight: 500;">
                                            <c:if test="${r.totalHorasDeuda > 0}">-</c:if><fmt:formatNumber value="${r.totalHorasDeuda}" type="number" maxFractionDigits="2"/>
                                        </td>

                                        <td style="text-align: center;">${r.diasAsistidos}</td>
                                        <td style="text-align: center; ${r.diasFaltas > 0 ? 'color: #c62828; font-weight: bold;' : ''}">${r.diasFaltas}</td>
                                        <td style="text-align: center;">${r.diasFeriadosLaborados}</td>
                                    </tr>
                                </c:forEach>
                                <c:if test="${empty reporte}">
                                    <tr>
                                        <td colspan="9" style="text-align: center; padding: 40px; color: var(--md-sys-color-secondary);">
            ...
            <c:if test="${empty reporteGenerado}">
                 <div style="text-align: center; padding: 60px; color: var(--md-sys-color-secondary);">
                    ...
                    <p style="font-size: 1.1rem;">Seleccione filtros y haga clic en "Generar Resumen" para ver el reporte.</p>
                 </div>
            </c:if>
        </div>
